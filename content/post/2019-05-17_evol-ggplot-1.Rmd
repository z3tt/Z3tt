...
💡 **Sort your data according to the best or worst, highest or lowest value to make your graph easily readable—do not sort them if the categories have an internal logical orderings,  as age groupings!**

To increase the readability we are going to flip the coordinates (note that we could also switch x and y arguments in the `ggplot` call—<s>but this does not work for box plots so we use `coord_flip()`</s>it now does!). It is also a good practice to include the 0 in plots which we can force by adding `scale_y_continuous(limits = c(0, 90))`.
...
💡 **Since the latest version 3.x.x of `{ggplot2}` you can also flip the orientation by switch x and y arguments:**
...
g + geom_jitter(size = 2, alpha = 0.25, width = 0.2, height = 0)
```
...
g + geom_jitter(position = position_jitter(seed = 2019, width = 0.2, height = 0), size = 2, alpha = 0.25)
```
