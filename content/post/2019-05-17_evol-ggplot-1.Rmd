...
layout: post
showtoc: false
description: "In this series of blog posts, I provide step-by-step tutorials explaining how my visualizations have evolved from a typical basic ggplot. In the first episode, I transform a basic boxplot into a colorful and self-explanatory combination of a jittered dot stripplot and a lollipop plot."
...
* [📝 Post Scriptum: Mean versus Median](#ps)
...
df_ratios %>%
  ggplot(aes(x = x, y = y, fill = student_ratio)) +
    ...
    geom_text(aes(x = x, y = y, label = alpha.2), color = "white", size = 5.5,
    ...
    theme(legend.position = c(0.5, 0),
          legend.text = element_text(family = "Roboto Mono", size = 15),
          plot.caption = element_text(family = "Poppins", color = "gray50", size = 15),
          legend.title = element_text(family = "Poppins", size =  18),
...
💡 **Sort your data according to the best or worst, highest or lowest value to make your graph easily readable—do not sort them if the categories have an internal logical orderinsic,  as age groupings!**

To increase the readability we are going to flip the coordinates (note that we could also switch x and y arguments in the `ggplot` call—but this does not work for boxplots so we use `coord_flip()`). It is also a good practice to include the 0 in plots which we can force by adding `scale_y_continuous(limits = c(0, 90))`.
...
💡 **Since the latest version 3.x.x of `{ggplot2}` you can also flip the orientation by using the orientation argument `geom_*(orientation = "y")`**
...
g <- 
  ggplot(df_sorted, aes(x = region, y = student_ratio, color = region)) +
    ...
    theme(
      ...
      axis.text.x = element_text(family = "Roboto Mono", size = 20),
      panel.grid = element_blank()
    )
```

(Note that to include these fonts we make use of the <strike>[`extrafont` package](https://github.com/wch/extrafont)</strike> [`showtext` package](https://github.com/yixuan/showtext). This package allows for the use of system fonts and is especially simple to use in combination with [Google fonts](https://fonts.google.com/). You need to have (a) the fonts installed on your system, (b) the package `showtext` installed, (c) import the fonts by running `font_add_google()` and (d) preferably turn on the automatic use of its functionality with `showtext_auto()`.)


### 📊 The Choice of the Chart Type {#geoms}

We can add any `geom_` to our ggplot-preset `g` that fits the data (note that until now it is just an empty plot with pretty axes):

```{r geoms, echo=FALSE, fig.width = 10, fig.height = 6.5}
library(patchwork)
...
(g + geom_point(size = 1) + labs(title = "Dot strip plot", subtitle = "g + geom_point(size = 1)")) &
  theme(
    plot.title = element_text(face = "bold", size = 24, margin = margin(b = 0)),
    plot.subtitle = element_text(size = 18, margin = margin(t = 5, b = 10)),
...
💡 **You can also set the seed within the `geom_jitter()` call by setting `position = position_jitter(seed)`. Note that in this case the `width` and/or `height` argument needs to be placed inside the position_jitter() as well:**

```{r plot-jitter-seed, eval = F}
...
(g_text <- 
  ...
  geom_jitter(size = 2, alpha = 0.25, width = 0.2) +
  annotate(
    "text", x = 6.3, y = 35, family = "Poppins", size = 5, color = "gray20", lineheight = .6,
    label = glue::glue("Worldwide average:\n{round(world_avg, 1)} students per teacher")
  ) +
  annotate(
    "text", x = 3.5, y = 10, family = "Poppins", size = 5, color = "gray20",
    label = "Continental average"
  ) +
  annotate(
    "text", x = 1.7, y = 11, family = "Poppins", size = 5, color = "gray20",
    label = "Countries per continent"
  ) +
  annotate(
    "text", x = 1.9, y = 64, family = "Poppins", size = 5, color = "gray20", lineheight = .6, 
...
after <- 
  ...
  ggplot(aes(x = region, y = student_ratio, color = region)) +
    ...
    theme_light(base_size = 24, base_family = "Poppins") +
    theme(
      legend.position = "none",
      plot.caption = element_text(size = 15, family = "Poppins", color = "gray50"),
...
median <- 
  ...
  ggplot(aes(x = region, y = student_ratio, color = region)) +
    ...
    annotate(
      "text", x = 1.7, y = 9.5, family = "Poppins", size = 5, color = "gray20",
      label = "Countries per continent"
    ) +
    annotate(
      "text", x = 1.9, y = 64, family = "Poppins", size = 5, color = "gray20", lineheight = .6,
    ...
    theme(
      legend.position = "none",
      plot.caption = element_text(size = 9, family = "Poppins", color = "gray50"),
      plot.title = element_text(size = 30, face = "bold", hjust = 0.5),
      axis.title = element_text(size = 24),
      axis.text.x = element_text(family = "Roboto Mono", size = 20),
      panel.grid = element_blank()
