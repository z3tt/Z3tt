...
* to include additional tips on e.g.:
...
(The number of elelements may vary depending on how you group them and whom you ask.)
...
The syntax of `{ggplot2}` is different from base R. As shown before, we always start to define a plotting element by calling `ggplot(data = df, aes(x = variable1, y = variable2))` which just tells `{ggplot2}` that we are going to work with that data. Thus, only a panel is created when running this because `{ggplot2}` does not know *how* we want to plot that data.
...
Let's tell `{ggplot2}` which style we want to use:
...
And let's get rid of the greyish default `{ggplot2}` look by setting a different built-in theme, e.g. `theme_bw()` - by calling `theme_set()` all following plots will have the same black'n'white theme. The red points look way better now!
...
Using `angle` and `vjust` you can adjust the position of the text (0 = left-aligned, 0.5 = centered, 1 = right-aligned):
...
Sometimes you want to zoom into your data. You can do this without subsetting your data:
...
Alignement is controlled by `hjust` (which stands for horizontal adjustment):
...
Note that you can also use different fonts. To use fonts which are installed on your machine (and you may be using in your office program) we get help from a package called `{extrafont}`. After we loaded the package, you need to import and load the fonts of installed on your device:
...
(You can also set a non-default font for all text elements of your plots, for more details see section ["Working with Themes"](#themes). I am going to use *Roboto Condensed* as new default font for the following plots.)
...
**Voila!** Now, we have a plot with grey lines and red pints as well as a single gray line and a single red point as legend symbols:
...
We can change the order by "dividing" both plots (and note the alignement even though one has a legend and one doesn't!):
...
Tableau is a famous visualiztion software with a well-known color palette. For R user it is available via the `{ggthemes}` package by calling `scale_color_tableau()`:
...
You can easily specify the overlap and the trailing tails by using the arguments `rel_min_height` and `scale`, respectively. The package also comes with  it's own theme (but I would prefer to build my own, see chapter ["Create and Use Your Custom Theme"](#themes)). Additionally, we change the colors based on year to make it more appealing.
...
[Plotly](https://plot.ly/feed) is a great tool for easily creating online, interactive graphics and web apps directly from your `{ggplot2}` plots. The workflow using the [`{plotly}` package](https://plot.ly/r/getting-started/) is surprisingly easy and [can be done from within R](https://plotly-r.com/).
...
* ["Cookbook for R"](http://www.cookbook-r.com/Graphs/), an online and printed book by Winston Chang with recipes to quickly produce the desired plot.
* Gallery of the [Top 50 ggplot visualizations](http://r-statistics.co/Top50-Ggplot2-Visualizations-MasterList-R-Code.html)
* Gallery of [`{ggplot2}` extension packages](http://www.ggplot2-exts.org/gallery/)
* [How to extend `{ggplot2}`](https://cran.r-project.org/web/packages/ggplot2/vignettes/extending-ggplot2.html)
* [#TidyTuesday](https://github.com/rfordatascience/tidytuesday), a weekly data project aimed at exploring and visualizing a data set that is provided by the [R4DS Online Learning Community](https://www.rfordatasci.com/) - check also the [#TidyTuesday hashtag on Twitter](https://twitter.com/hashtag/TidyTuesday?lang=en) and [this collection of contributions by Neil Grantham](https://nsgrantham.com/tidytuesdayrocks/)
