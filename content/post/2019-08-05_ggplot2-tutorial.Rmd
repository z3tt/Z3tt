...
layout: post
showtoc: false
description: "This is a rework of the blog entry called 'Beautiful plotting in R: A ggplot2 cheatsheet' by Zev Ross, posted on 4. August 2014, updated last in January 2016. Most changes were made to have an updated version, to follow the R style guide, to change style and aesthetics of plots to be (more) beautiful and meaningful as well as to include additional tipps."
...
Since the blog entry by Zev Ross was not updated for some years, I hosted the updated version (last update Jan 2017) on my GitHub. Now it finds its proper place on this homepage!
...
ggplot(chic, aes(date, temp)) +
  ...
  labs(x = "Year", y = "Temperature (°F)") + 
  theme(plot.background = element_rect(fill = "gray60"),
        plot.margin = unit(c(1, 5, 1, 5), "cm"))  ### top, right, bottom, left => TRouBLe
```
...
Not only boring but uninformative. One could add transparency to deal with overplotting:
