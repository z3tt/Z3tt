...
* [💬 Add Text Boxes to Let The Plot Speaks for Itself](#text)
...
💡 **Sort your data according to the best or worst, highest or lowest value do make your graph easily readable - do not sort them if the categories have an internal logical order such as age groups!**
...
* let the axis start at 0 and reducing the spacing to the plot margin by adding `expand = c(0.005, 0.005)` as argument to the `scale_y_continious()`,
...
* do a some adjustedment of the new theme (e.g. changing some font settings and removing the legend and grid) by adding `theme()`.
...
I am going to save the ggplot call and the all these visual adjustments in `gg` object called `g` so we can use it for the next plots.
...
(Note that to use this font we make use of the [`extrafont` package](https://github.com/wch/extrafont). You need to have (a) the font installed on your system, (b) the package `extrafont` installed, (c) import your fonts´by running `extrafont::font_import()` and (d) load them via `extrafont::loadfonts(device = "win")`)
...
(g + geom_point(size = 2, alpha = 0.15) + labs(title = "Dot strip plot",
                                               subtitle = "g + geom_point(size = 2, alpha = 0.15)")) &
...
```{r box-dot-plot, echo=-1, fig.width = 8, fig.heigh = 4.5}
set.seed(123)
...
g7 <- g + geom_point(size = 3, alpha = 0.15)

g8 <- g + geom_boxplot(color = "grey50", outlier.alpha = 0) +
  geom_point(size = 3, alpha = 0.15)
...
set.seed(123)
g10 <- g + geom_jitter(size = 2, alpha = 0.25, width = 0.2)

set.seed(123)
g11 <- g + geom_point(aes(region, student_ratio_region), size = 5) +
  geom_jitter(size = 2, alpha = 0.25, width = 0.2)

set.seed(123)
g12 <- g + geom_hline(aes(yintercept = world_avg), color = "grey70", size = 0.6) +
  geom_point(aes(region, student_ratio_region), size = 5) +
  geom_jitter(size = 2, alpha = 0.25, width = 0.2)
...
g17 <- g16 + annotation_custom(ggplotGrob(map_regions), xmin = 2.5, xmax = 7.5, ymin = 55, ymax = 85)
...
saveGIF({
  ...
  print(g16)
  print(g17)
  print(g17 + labs(caption = caption) + theme(plot.caption = element_text(size = 7, family = "Poppins", color = "grey50")))
  print(g17 + labs(caption = caption) + theme(plot.caption = element_text(size = 7, family = "Poppins", color = "grey50")))
  print(g17 + labs(caption = caption) + theme(plot.caption = element_text(size = 7, family = "Poppins", color = "grey50")))
