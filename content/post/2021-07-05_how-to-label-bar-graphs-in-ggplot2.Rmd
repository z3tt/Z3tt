...
With the help of the `{glue}` package, you can also go crazy and add the manufacturer itself to the label!

```{r perc-label-custom-manufacturer}
mpg_sum %>% 
  dplyr::mutate(
    ## add percentage label with `sprintf()`
    perc = paste0(sprintf("% 2.1f", n / sum(n) * 100), "%"),
    ## customize label for the top 3
    perc = if_else(
      row_number() %in% 1:3, 
      glue::glue(perc, "of all car models are {manufacturer}"), perc)
  ) %>%
