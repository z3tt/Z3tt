...
Now we can run the functional manually for each manufacturer featured in the data set:
...
```
<figcaption>We now can manually create plots for specific manufacturers without the need to copy-paste and adjust the ggplot code manually.</figcaption>
...
Let's consider an example in which we vary both positional variables, using the 2008 car fule data:

```{r purrr-function-scatter-lm, eval=FALSE}
map2(c("displ", "displ", "hwy"), c("hwy", "cty", "cty"),
...
pmap(
  names_set, ~plot_scatter_lm(data = palmerpenguins::penguins, 
...
wrap_plots(pmap(names_set,
           ~plot_scatter_lm(
             data = palmerpenguins::penguins, var1 = .x, var2 = .y, color = "species"
           ) + theme(plot.margin = margin(rep(15, 4)))))
```
